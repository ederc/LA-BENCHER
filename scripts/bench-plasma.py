...
if int(args.inc) == -1:
  ...
  for i in threads:
    print(strstr+'-t'+str(i)+' -m5>> '+bench_file+'...')
    os.system(strstr+'-t'+str(i)+' -m5 >> '+bench_file)
...
# works only for GEP
  ...
  for k in range(0,int(args.count)+1):
    ...
    print(strstr+' -t'+str(max_threads)+' -m5 >> '+bench_file+'...')
    os.system(strstr+' -t'+str(max_threads)+' -m5 >> '+bench_file)
    print 'Done at '+time.strftime("%a, %d %b %Y %H:%M:%S +0000", time.gmtime())
